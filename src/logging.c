...
static inline const char *FASTEST_ErrorToString(uint64_t exit_status, uint64_t flags) {
    if (exit_status & FASTEST_SUCCESS) return "SUCCESS";

    if (exit_status & FASTEST_ERROR_ASSERT) {
        if (flags & FASTEST_ASSERT_EQ) return "ASSERT_EQ failed";
        if (flags & FASTEST_ASSERT_NEQ) return "ASSERT_NEQ failed";
        if (flags & FASTEST_ASSERT_GT) return "ASSERT_GT failed";
        if (flags & FASTEST_ASSERT_GE) return "ASSERT_GE failed";
        if (flags & FASTEST_ASSERT_LT) return "ASSERT_LT failed";
        if (flags & FASTEST_ASSERT_LE) return "ASSERT_LE failed";
        return "ASSERT failed";
    }

    if (exit_status & FASTEST_ERROR_UNEXPECTED) return "Unexpected result";
    if (exit_status & FASTEST_ERROR_EXCEPTION) return "Exception occurred";
    if (exit_status & FASTEST_ERROR_MEMORY) return "Memory error";
    if (exit_status & FASTEST_ERROR_TIMEOUT) return "Timeout";
    if (exit_status & FASTEST_ERROR_RESOURCE) return "Resource error";
    if (exit_status & FASTEST_ERROR_MPI) return "MPI error";
    if (exit_status & FASTEST_ERROR_OMP) return "OpenMP error";
    if (exit_status & FASTEST_ERROR_CUDA) return "CUDA error";
    if (exit_status & FASTEST_ERROR_INTERNAL) return "Internal framework error";
    if (exit_status & FASTEST_ERROR_UNKNOWN) return "Unknown error";
